// Headless harness.
//   npx tsx experiments/lab/run.ts [--ex 001,002] [--cond baseline,unicode|all] [--decider jev,claude-haiku] [--repeats 3] [--max 60] [--quiet] [--jobs 6] [--resume]
...
const deciderNames = opt('decider', 'jev').split(',')
...
export interface LedgerRow {
  stamp: string; file: string; exercise: string; condition: string; repeat: number; decider: string; model: string
...
async function one(ex: Exercise, cond: Condition, rep: number, decider: Decider): Promise<LedgerRow> {
  const stamp = new Date().toISOString().replace(/[:.]/g, '-')
  const turns: TurnRecord[] = []
  let final: Score = ex.score, tokens = 0, modelMs = 0, stopped = false, capped = false, cost = 0
  let errorsAtFirstComplete: number | null = null, turnsToComplete: number | null = null, stalled = false
  const t0 = performance.now()
  if (!quiet) console.log(`\n# ${ex.id} · ${cond.name} · ${decider.name} · repeat ${rep}`)
  for await (const ev of runLoop(decider, ex, ex.score, { condition: cond, maxTurns })) {
    if (ev.type === 'step') {
      tokens += ev.rec.inputTokens; modelMs += ev.rec.ms; cost += ev.rec.costUsd
  ...
  const file = `runs/${stamp}-${ex.id}-${cond.name}-${decider.name}-r${rep}.json`
  const row: LedgerRow = {
    stamp, file, exercise: ex.id, condition: cond.name, repeat: rep, decider: decider.name, model: decider.model,
    turns: turns.length, stopped, capped, stalled, tokens, costUsd: cost, modelMs, wallMs,
  ...
  writeFileSync(file, JSON.stringify({ exercise: ex.id, condition: cond, decider: decider.name, repeat: rep, turns, final: serializeScoreBlock(final, cond), report, row }, null, 1))
  appendFileSync('runs/index.jsonl', JSON.stringify(row) + '\n')
  console.log(`${ex.id} · ${cond.name} · ${decider.name} · r${rep}: ${turns.length} turns${stopped ? ' STOP' : stalled ? ' STALLED' : capped ? ' CAPPED' : ''} · ${report.errors} err / ${report.warnings} warn · chords ${(row.chordRate * 100).toFixed(0)}% · pitch conf ${row.meanPitchConf.toFixed(2)} · $${row.costUsd.toFixed(4)}`)
  if (!quiet) console.log(formatReport(report))
  return row
}

const done = new Set<string>()
if (resume && existsSync('runs/index.jsonl')) for (const l of readFileSync('runs/index.jsonl', 'utf8').split('\n')) if (l) { const r = JSON.parse(l); done.add(`${r.exercise}|${r.condition}|${r.decider ?? 'jev'}|${r.repeat}`) }
const deciders = await Promise.all(deciderNames.map(deciderByName))
const queue: [Exercise, Condition, number, Decider][] = []
for (const d of deciders) for (const ex of exercises) for (const cond of conds) for (let r = 1; r <= repeats; r++) if (!done.has(`${ex.id}|${cond.name}|${d.name}|${r}`)) queue.push([ex, cond, r, d])
console.log(`${queue.length} runs queued (${done.size} already done), ${jobs} in parallel`)
await Promise.all(Array.from({ length: jobs }, async () => { for (;;) { const job = queue.shift(); if (!job) return; try { await one(...job) } catch (e) { console.error(`FAILED ${job[0].id} ${job[1].name} ${job[3].name} r${job[2]}: ${e}`) } } }))
